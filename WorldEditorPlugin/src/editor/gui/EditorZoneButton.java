...
public final class EditorZoneButton extends MouseAdapter {
    ...
    public void mousePressed(MouseEvent event) {
        ...
        if(pressedButton == MouseEvent.BUTTON1) {
            ...
            guiButton.setIcon(WorldZone.getZoneFromName(activeZoneName).editorTexture.get());

            if(WorldZone.getZoneFromName(activeZoneName) instanceof WorldZoneTexturable) {
                guiButton.setIcon(((WorldZoneTexturable) WorldZone.getZoneFromName(zoneID)).textureVariants.get()[editorInstance.textureSelector.activeMaterial.index]);
            ...
            if(WorldZone.getZoneFromName(zoneID) instanceof WorldZoneTexturable) {
                guiButton.setIcon(((WorldZoneTexturable) WorldZone.getZoneFromName(zoneID)).textureVariants.get()[editorInstance.textureSelector.activeMaterial.index]);
                zoneTexture = editorInstance.textureSelector.activeMaterial.name;
            }
        }
